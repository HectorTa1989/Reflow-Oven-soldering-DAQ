...
/*	wChRange[0] = NEG_50_TO_50_MV;		
	...
	wChRange[0] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[1] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[2] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[3] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[4] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[5] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[6] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[7] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[8] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[9] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[10] = TYPE_K_NEG_270_TO_1372_C;
	wChRange[11] = TYPE_K_NEG_270_TO_1372_C;
...
//  sprintf(filename2, "%04d%02d%02d_%02dh%02dm%02ds.csv", 
 //         timeinfo->tm_year, timeinfo->tm_mon, timeinfo->tm_mday, timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec );
  ...
  sprintf(filename2, "%04d%02d%02d_%02dhs.csv", 
          timeinfo->tm_year, timeinfo->tm_mon, timeinfo->tm_mday, timeinfo->tm_hour);
  printf("\nFilename: %s", filename2);


  ofstream dest2;
 dest2.open(filename2, ios_base::app | ios_base::out);
...
//          timeinfo->tm_year, timeinfo->tm_mon, timeinfo->tm_mday, timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec );
	  sprintf(filename, "%04d%02d%02d_%02dhs.csv", 
          timeinfo->tm_year, timeinfo->tm_mon, timeinfo->tm_mday, timeinfo->tm_hour);
		printf("\nFilename: %s", filename);
...
//  while(1){
	...
	//	dest<<timeinfo->tm_year<<"-"<< timeinfo->tm_mon<<"-"<< timeinfo->tm_mday<< ","<< timeinfo->tm_hour<<"h"<<timeinfo->tm_min<<"m"<<timeinfo->tm_sec<<"s"<<",";
...
/*
				v.push_back(dScaledValue);
				for(vector<WORD>::const_iterator i = v.begin(); i != v.end(); i++)
					dest<< *i<<",";
*/

				dest<< dScaledValue <<",";
				
		//		dest<<timeinfo->tm_hour<<"h"<<timeinfo->tm_min<<"m"<<timeinfo->tm_sec<<"s"<<","<<"Channel "<<iCnt<<","<< dScaledValue<<","<<endl;						
				//Sleep(1000);
				Sleep(1);
			}
			else
...
//	struct tm *now = localtime_s(&t);
	struct tm *now ;
	char cur_time[80];
	strftime(cur_time, sizeof(cur_time), "D:\Projects\Reflow oven\%Y-%m-%d.csv",now);
//	asctime_s(cur_time,sizeof(cur_time),localtime_s(&t, &now));
	
//	ofstream outFile("D:\Projects\Reflow oven\ThermoDAQ.csv");
	ofstream outFile;
//	outFile.open("D:\Projects\Reflow oven\ThermoDAQ.csv");
//	outFile.open(file);
//	outFile << "Writing temperature data to ThermoDAQ file. \n";
...
//		fprintf(outFile,"%d\t, %d\t, %d",);
		outFile<< "Channel" << "Temperature" << ", " << endl;
	}
	outFile.close();
...
//	char cUnit[3] = {0};
 ...
 // sprintf(filename, "D:\Projects\Reflow oven\Thermo1\Thermo1\%02d%02d%02d.csv", 
  ...
  ofstream dest;
  dest.open(filename);
//  dest<< "Channel" <<","<< "Temperature" << ", " << endl;
  dest<< "Channel 1" << ","<<"Channel 2"<< ","<<"Channel 3"<< ","<<"Channel 4"<< ","<<"Channel 5"<< ","<<"Channel 6"<< ","<<"Channel 7"<< ","<<"Channel 8"<< ","<<"Channel 9"<< ","<<"Channel 10"<< ","<<"Channel 11"<< ","	<<"Channel 12" << ", "<< endl;
  for (int iCnt = 0; iCnt < g_ChannelNum ; iCnt++)
	{		
		dest<< dScaledValue<<","<<endl;
...
int main(int argc, char* argv[])
{
	...
	//==============================
	WORD wSlotID = 9; //the slot ID which is ranged from 0 to 31
